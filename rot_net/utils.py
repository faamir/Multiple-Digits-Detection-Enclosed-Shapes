...
"""from https://github.com/d4nst/RotNet"""
...
def angle_error_regression(y_true, y_pred):
    ...
    """
    return K.mean(angle_difference(y_true * 45, y_pred * 45))
...
class RotNetDataGenerator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        batch_x = np.zeros(
            (len(index_array),) + self.input_shape, dtype="float32"
        )
        ...
        for i, j in enumerate(index_array):
            ...
            if self.rotate:
                # get a random angle
                rotation_angle = np.random.randint(45)
            else:
                rotation_angle = 0
        ...
        if self.one_hot:
            ...
            batch_y = to_categorical(batch_y, 45)
        else:
            batch_y /= 45
